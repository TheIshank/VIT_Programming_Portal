 ...
 */
	...
	if(isset($_POST['action'])){
		if($_POST['action']=='email') {
			...
			if(trim($_POST['title']) == "" or trim($_POST['problem']) == "" or !is_numeric($_POST['time']))
				header("Location: problems.php?derror=1");
			       ...
			       $query = "SELECT max(Problem_Id) as id1 from problems";
				   	$result = mysql_query($query);
				  ...
				  $inputPath="admin-login/$problemId/input.txt";
				  $outputPath="admin-login/$problemId/output.txt";
				  $hiddeninputPath="admin-login/$problemId/hiddenInput.txt";
				  $hiddenoutputPath="admin-login/$problemId/hiddenOutput.txt";
				  $precodePath="admin-login/$problemId/precode.txt";
				  $postcodePath="admin-login/$problemId/postcode.txt";
				  $descPath="admin-login/$problemId/description.txt";
				
				
				
				$query="INSERT INTO `problems` ( `problem_title` , `description_path`, `test_case_input_path`, `test_case_output_path`, `hidden_case_input_path`, `hidden_case_output_path`,`precode_path`,`postcode_path`, `execution_time`, `lang_freeze`, `test_id`, `difficulty_level`, `maximum_marks`) VALUES ('".mysql_real_escape_string($_POST['title'])."','$descPath', '$inputPath', '$outputPath', '$hiddeninputPath','$hiddenoutputPath','$precodePath', '$postcodePath', '".$_POST['time']."', '".mysql_real_escape_string($_POST['lang_freeze'])."', '".$_POST['testId']."', '".$_POST['difficulty_level']."', '".$_POST['max_marks']."')";
				mysql_query($query);
			...
			if(trim($_POST['title']) == "" or trim($_POST['problem']) == "" or !is_numeric($_POST['time']))
             ...
             ////YAHAN SE CHANGE KAR............!!!!!!!!!!!!!! CHAL BYE!!!!!!!!!!


				
				//$query = "UPDATE problems SET input="'$path'.'input.txt'" where sl='".$_POST['id']."'";
				$query = "UPDATE problems SET problem_title='".mysql_real_escape_string($_POST['title'])."', Execution_Time='".$_POST['time']."', test_id='".$_POST['testId']."', difficulty_level='".$_POST['difficulty_level']."', maximum_marks='".$_POST['max_marks']."', lang_freeze='".mysql_real_escape_string($_POST['lang_freeze'])."' WHERE Problem_Id='".$_POST['id']."'";
		...
		if($_GET['action']=='delete' and is_numeric($_GET['id'])) {
			...
			$query="DELETE FROM problems WHERE problem_id=".$_GET['id'];
			mysql_query($query);
			$query="DELETE FROM submissions WHERE problem_id=".$_GET['id'];
			mysql_query($query);
